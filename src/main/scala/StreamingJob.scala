...
class StreamingJob() extends Serializable {
  ...
  def extractValues(avroRecord: Object, selectedFields: Array[String]):
    scala.collection.mutable.Map[String, String] = {

    // Create an empty map that will contain the mappings (fieldName -> fieldValue) by fetching its
    // value from inside the record
    ...
    selectedFields.foreach { fieldName =>
      // Split the field name according to the avro nested structure flattening convention,
      // "." in our case
      println(fieldName)
      val steps = fieldName.split("\\.").map(_.trim)
      val depth = steps.length
      println(s"Depth: ${depth}")
      // Creation of the field value as a var since we will interatively go down the steps to
      // grab the desired field (leaf)
  ...
  // Function that uses the name to value mapping extracted from the avro record
  // on the user-specified filter
  def applyMap2Filter(filter: Array[Any], map: scala.collection.mutable.Map[String,String]):
    Boolean = {
    ...
    val mappedFilter = filter.map { c =>
      c match {
        ...
        case array: Array[Any] =>
          ...
          // start the fold with false: any boolean b is invariant by "false || b" operation
          // Note that .foldLeft(false)(_ || _) even works for Array.empty[Boolean],
          // while a reduce(_ || _) does not
        case _ =>
          println("Unconventional filter. Read the doc.")
          throw new IllegalArgumentException
      }
    }.foldLeft(true)(_ && _)
    // start the fold with true: any boolean b is invariant by "true && b" operation
    // Note that .foldLeft(true)(_ && _) even works for Array.empty[Boolean],
    // while a reduce(_ && _) does not
    mappedFilter
  }

  // Function that uses the two previous auxiliary functions to calibrate a DStream
  // for what is specified in the config
  def calibrate(stream: DStream[(Object,Object)], config: StreamingJobConfig):
    ...
    val calibratedStream = stream.map(_._2).filter { avroRecord =>
      ...
      // The returned object is a mapping (fieldName -> fieldValue), for all fieldName
      // in config.selectedFields
      val extractionMap = extractValues(avroRecord, config.selectedFields)
      // Now that the values of the selected fields have been extracted from the current
      // avro record, we generate a Boolean which answers the question:
      ...
      // The output is a map because it enables easy grabbing of any values knowing
      // the field name
  ...
  // The fields containing time information has to be specified to dsiplay the content
  // as a time series
  def sendGrafana(stream: DStream[Map[String, String]], config: StreamingJobConfig,
    timeField: String, valueField: String, metric: String): Unit = {
  ...
  // Function that returns the DStream whose shape is specified in the StreamingJobConfig
  // object passed in parameter
  def subscribeToKafkaStream(ssc: StreamingContext, myStreamingJobConfig: StreamingJobConfig):
    DStream[Map[String, String]] = {

    val brokerList = myStreamingJobConfig.kafkaBrokerHost + ":" +
      myStreamingJobConfig.kafkaBrokerPort.toString
    val schemaRegistry = "http://" + myStreamingJobConfig.schemaRegistryHost + ":" +
      myStreamingJobConfig.schemaRegistryPort.toString
    ...
    val dataStream = KafkaUtils.createDirectStream[Object,
                                                   Object,
                                                   KafkaAvroDecoder,
                                                   KafkaAvroDecoder](ssc, kafkaParams, topicSet)
