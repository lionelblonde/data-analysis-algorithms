...
class AnomalyDetection() {
  ...
  def run(stream: DStream[Map[String, String]], config: AnomalyDetectionConfig):
    ...
    // Interval after which an RDD is generated in DStream
    val slide: Duration = Seconds(config.slidingInterval)
    ...
    val slideStream = stream_.map(x => (x._1, x._2, 1.0)).reduceByWindow(
      ...
      // (most recent timestamp, concatenation of the values, sum over slide, count over slide)
      val timestamp = x._1
      val mean = x._2/x._3
      (timestamp, mean)
    }
